...
import pandas as pd
# ---- Lock to GPU-0 (must be before importing torch/tensorflow/etc.) ----
import os
...
if input_option == "Manual Input":
    ...
    if st.button("Generate Response"):
        if prompt and report:
            response = generate_response("llama3.3:latest", prompt, report, json_template)
            st.write("### Response:")
            st.write(response)
        else:
    ...
    if uploaded_file:
        ...
        if "text" in df.columns:
            ...
            if st.button("Process CSV"):
                df["response"] = df["text"].apply(lambda report: generate_response("llama3.3:latest", prompt, report, json_template))
                st.write("### Processed Responses:")
                st.dataframe(df)
                
                # Create a separate DataFrame with prompt_type and response
                response_df = pd.DataFrame({
                    "prompt_type": ["Cloze"] * len(df),
                    "response": df["response"]
                })
                
                # Provide download button for the response as JSON file
                json_data = response_df.to_json(orient="records", indent=2)
                st.download_button("Download JSON", json_data, "morph_responses_lama3_p2.json", "application/json")
                
                # Provide download button for the response as CSV file
                st.download_button("Download CSV", response_df.to_csv(index=False), "morph_responses_lama3_p2.csv", "text/csv")
        else:
