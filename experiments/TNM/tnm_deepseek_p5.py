    ...
    if uploaded_file:
        ...
        if "text" in df.columns:
            ...
            if st.button("Process CSV"):
                ...
                st.download_button("Download JSON", json_data, "tnm_responses_deepseek_p5.json", "application/json")
                
                # Provide download button for the response as CSV file
                st.download_button("Download CSV", response_df.to_csv(index=False), "tnm_responses_deepseek_p5long_term_memory.py.csv", "text/csv")
        else:
