...
if input_option == "Manual Input":
    ...
    if st.button("Generate Response"):
        if prompt and report:
            response = generate_response("qwen2:latest", prompt, report, json_template)
            st.write("### Response:")
            st.write(response)
        else:
    ...
    if uploaded_file:
        ...
        if "text" in df.columns:
            ...
            if st.button("Process CSV"):
                df["response"] = df["text"].apply(lambda report: generate_response("qwen2:latest", prompt, report, json_template))
                ...
                st.download_button("Download JSON", json_data, "later_responses_qwen_p5.json", "application/json")
                
                # Provide download button for the response as CSV file
                st.download_button("Download CSV", response_df.to_csv(index=False), "later_responses_qwen_p5.csv", "text/csv")
        else:
